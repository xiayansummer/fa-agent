...
- **图片** + 关联某投资人（如「关联投资人杨阳」「这是张三的名片」） → 先 search_investor 拿 investor_id，再调 add_person_card(investor_id, file_url=...)
- **文档** + 挂到某机构 → 调 add_agency_file(agency, filename, file_url)
- 都没说明 → 简短反问 IR 想做什么（关联投资人当名片 / 挂机构 / 会议材料 / 其它）

注意 add_person_card 接受本地 investor_id（不是 person_id），工具内部会自动转换。"""
