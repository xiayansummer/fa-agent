...
⚠️ **写操作必须真实调用工具**：没有调用工具就绝对不允许回复「✅ 已写入/已记录/已保存」——
那等于骗 IR 数据已保存、实际什么都没存（系统会拦截这种回复并要求 IR 重发）。要写就调工具，调完按下面规则回报。
涉及日期的回复，**星期几一律以工具返回的 weekday 为准**，严禁自己推算（你算不准）。
**年份以「当前时间」为准**：IR 说「6/16」就是当前年份的 06-16（已过则取下一年），
绝不要凭你的记忆写成 2025 等过去年份——你的内在时间感是滞后的，以系统给的当前时间为唯一事实。
...
async def _chat_create(**kwargs):
    """chat LLM 调用统一入口：opencode 网关对 qwen3.7-plus 工具调用偶发连接错误/5xx
    （bench ~20%），重试 2 次（1s/2s 退避）后仍失败才抛。"""
    ...
    for attempt in range(3):
        try:
            ...
            last = e
            logger.warning("chat llm call failed (attempt %d/3): %s", attempt + 1, e)
...
# 写类工具：调用成功(ok=True)才算"真的写了"
_WRITE_TOOLS = {
    "add_calendar_event", "record_interaction", "add_agency_summary",
    "add_person_summary", "set_investor_familiarity", "set_investor_tags",
    "bind_business_card", "add_person_card", "add_agency", "add_agency_file",
    "schedule_tencent_meeting", "cancel_tencent_meeting",
}
_FAKE_WRITE_RE = re.compile(r"[✅✔][^\n]{0,6}已(写入|记录|保存|新建|添加|加)")


def _guard_fake_write(reply: str, wrote: bool, ir_id: int) -> str:
    """LLM 偶发不调工具直接回「✅ 已记录」（2026-06-12「复旦科创脑机竞赛」实锤：
    嘴上已记录、库里无此行）。服务端兜底：声称写入但本轮零成功写调用 → 拦截改写，
    幻觉不允许到 IR 眼前。"""
    if wrote or not reply or not _FAKE_WRITE_RE.search(reply):
        return reply
    logger.warning("chat fake-write intercepted ir=%s reply=%s", ir_id, reply[:120])
    return ("⚠️ 刚才的保存操作没有真正执行（系统已拦截一次无效回复）。"
            "麻烦把要记录的内容再发一次，我会重新保存并回报结果。")
...
async def free_chat(
    ...
    system_prompt = (
        f"{_SYSTEM_PROMPT}\n\n"
        f"当前时间：{now.strftime('%Y-%m-%d %H:%M:%S')}（{_WEEKDAYS[now.weekday()]}）(Asia/Shanghai)\n"
    ...
    wrote_ok = False  # 本轮是否有「写类工具且返回 ok」——假写拦截的依据
    ...
    for step in range(8):
        ...
        if not tool_calls:
            return ChatResponse(
                reply=_guard_fake_write(msg.content or "", wrote_ok, ir_id),
        ...
        for tc in tool_calls:
            ...
            if fname in _WRITE_TOOLS and isinstance(result, dict) and result.get("ok"):
                wrote_ok = True
    ...
    try:
        ...
        reply = _guard_fake_write(final_resp.choices[0].message.content or "", wrote_ok, ir_id)
